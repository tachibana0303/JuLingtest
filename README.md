...
### 个人角色与贡献（@tachibana0303）

本项目中，我主要负责以下关键逻辑的理解、设计和反复调优：

**1. 物品紧贴墙壁的精确位置调整**
- 通过 edgeNormal() 计算墙壁法向量（始终指向区域内部），结合物品自身尺寸，将物品中心坐标精确偏移至紧贴墙面的位置
- 针对斜墙场景，利用线段参数化 pointOnEdge() 计算沿墙的偏移量，确保物品沿斜面紧密排列

**2. 门开口方向与开门区域的碰撞处理**
- 解析 isOpenInward 属性，区分内开门（绿色虚线区域）和外开门（黄色虚线区域）
- getFridgeSwingCorners() 根据冰箱开口方向计算双开门摆动区域，与货架位置进行碰撞检测
- 图四中 shelf-2 放置在右墙时，自动检测是否与冰箱开门区域重叠，若有碰撞则向下微调

**3. 四图各自的布局方案设计**
- 每张图的边界形状、门位置、物品种类各不相同，需要针对空间特征独立设计摆放策略
- 图一（不规则储物区）：左斜墙紧贴排列 + 右墙货架竖放 + 顶墙离地架旋转 90° 并排
- 图二（储藏区）：制冰机与冰箱安全距离控制 + 右墙货架堆叠 + 底部离地架组合
- 图三（长条型储藏区）：冰箱顶部居中 + 左墙 5 货架垂直排列 + 内开门绿色摆动区域
- 图四（储物区）：冰箱长边沿左墙 + shelf-2 自动避让开门区域 + 底墙 3 离地架并排

**4. 碰撞检测逻辑**
- 检测冰箱开门摆动区域与货架位置的交叉，超出区域时触发向下微调
- 确保所有物品不超出边界轮廓，且不与门摆动区域重叠
